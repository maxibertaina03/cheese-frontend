...
function App() {
  ...
  const handleCut = async () => {
    if (!unidadParaCortar) {
      setError('No se seleccionó unidad');
      return;
    }
    ...
    // Validar que haya suficiente peso
    if (peso > unidadParaCortar.pesoActual) {
      setError(`Peso insuficiente. Disponible: ${unidadParaCortar.pesoActual}g`);
      return;
    }
  ...
  const handleEgresoTotal = async () => {
    ...
    const pesoTotal = unidadParaCortar.pesoActual;
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(`Corte registrado: ${pesoTotal}g`);
        setShowCutModal(false);
        setCodigoBarrasCorte('');
        setPesoCorte('');
        setObservacionesCorte('');
              ...
              {unidadParaCortar.pesoActual > 0 && (
                <div className="form-group">
                  <button
                    className="btn-action btn-cut"
                    onClick={() => {
                      // 🎯 Egreso directo sin validaciones
                      handleEgresoTotal();
                    }}
                  >
